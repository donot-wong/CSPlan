...
class DistributeConsumer(ConsumerBase):
    def __init__(self, ampq_url, queue_name, routing_key, q):
    ...
    def on_message(self, unused_channel, basic_deliver, properties, body):
        ...
        self.transQueue.put({'routing_key': 'sqliscan.key', 'body': body})
        self.acknowledge_message(basic_deliver.delivery_tag)


def distributeMain(q):
    example = DistributeConsumer('amqp://guest:guest@localhost:5672/%2F', 'distribute', 'distribute.source', q)
    try:
        example.run()
...
def distributeTrans(q):
    publishmultiObj = PublisherMultiBase('amqp://guest:guest@localhost:5672/%2F?connection_attempts=3&heartbeat_interval=3600', ['sqliscan'], ['sqliscan.key'], q)
    publishmultiObj.run()